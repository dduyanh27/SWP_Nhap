...
<div class="admin-toolbar">
    <div class="toolbar-left">
        <select id="sortSelect" name="sort" class="admin-select"
            ...
            <option value="stock_asc"  ${param.sort == 'stock_asc'  ? 'selected' : ''}>Stock ASC (Sort)</option>
            <option value="stock_desc" ${param.sort == 'stock_desc' ? 'selected' : ''}>Stock DESC (Sort)</option>
            <option value="price_asc"  ${param.sort == 'price_asc'  ? 'selected' : ''}>Price ASC (Sort)</option>
            <option value="price_desc" ${param.sort == 'price_desc' ? 'selected' : ''}>Price DESC (Sort)</option>
        </select>

        <select id="priceFilter" name="price" class="admin-select"
                onchange="applyFilters()">
            ...
            <option value="100_300"   ${param.price == '100_300'   ? 'selected' : ''}>100.000đ – 300.000đ</option>
            ...
            <c:choose>
                <c:when test="${not empty productList}">
                    <c:forEach var="p" items="${productList}">
                        <tr>
                            <td class="product-name">P${String.format("%03d", p.productId)}</td>
                            <td>
                            ...
                            <td>${p.price}đ / ${p.unit}</td>
                            <td class="product-name">${p.stock}</td>
                            <td>
                                <c:choose>
                                    ...
                                    <c:when test="${p.stock == 0}">
                                        <span class="status-badge hidden" style="background-color: #F1F5F9; color: #64748B;">Out of Stock</span>
                                    </c:when>
